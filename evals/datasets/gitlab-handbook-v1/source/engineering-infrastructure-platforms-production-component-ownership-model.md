---
slug: engineering-infrastructure-platforms-production-component-ownership-model
title: "Component Ownership Model"
source_url: "https://handbook.gitlab.com/handbook/engineering/infrastructure-platforms/production/component-ownership-model/"
...
## Signs you're doing Component Ownership Model wrong

The following anti-patterns indicate that a team is not following the Component Ownership Model correctly.
If you recognize any of these in your workflow, revisit the guidance in this document and engage with your Integration Engineer.

1. **Majority of work happening on ops.gitlab.net instead of gitlab.com.**
   The [ops instance](https://ops.gitlab.net) is not intended to be a general purpose instance for GitLab developers to work on.
   It is a special instance used for deploying our own infrastructure.
   If you need to do the majority of your infrastructure development on this instance, you're doing it wrong.
   Mirrors of your code will be maintained on ops.gitlab.net for disaster recovery purposes,
   but this instance should not be needed for development.

1. **Developers outside Infrastructure Platforms having to make major changes to infrastructure monorepos.**
   Repositories such as [Config-Mgmt](https://ops.gitlab.net/gitlab-com/gl-infra/config-mgmt) and
   [k8s-workloads](https://gitlab.com/gitlab-com/gl-infra/k8s-workloads/gitlab-com) are owned by Infrastructure Platforms.
   If you're writing code in these repositories, you don't own it, so you're not following the Component Ownership Model.
   Component Owners should maintain their own modules in separate repositories and integrate them as versioned dependencies.

1. **The Team driving development are not maintainers on the target repository.**
   If your team needs to ask Engineers in a different sub-department for approval for day-to-day,
   ownership has not been established correctly.
   Component Owners should have maintainer rights on their own component repositories.

1. **Primary artifact is not a Terraform Module or Helm Chart, or the Terraform Module is a root module.**
   Development teams should be building child Terraform modules which can be integrated into other Terraform projects, not root modules.
   Helm charts should integrate within a wider set of Kubernetes resources.

1. **Changes not linted or verified using standard tooling from [`common-ci-tasks`](https://gitlab.com/gitlab-com/gl-infra/common-ci-tasks).**
   All component modules must use the CI components provided by `common-ci-tasks`,
   including `Checkov`, `tflint`, Helm Unit, and `terraform test`.
   Skipping these checks undermines the policy-as-code guardrails that make the model work.

1. **Developers cannot deploy their changes in a sandbox for testing.**
   If the first time a change is planned or deployed is the staging environment, you're doing it wrong.
   Developers should be able to test against a [sandbox account](/handbook/company/infrastructure-standards/realms/sandbox/), or with tools such as [LocalStack](https://gitlab.com/gitlab-com/gl-infra/common-ci-tasks/-/tree/main/templates/localstack).

1. **Copypasta development: duplicating Terraform or Helm configurations instead of using modules.**
   Duplicate code is widely acknowledged to devolve into unmanageable code debt.
   You wouldn't duplicate source code for running in staging vs. production,
   and Terraform and Helm should be no different.
   If you're duplicating Terraform or Helm configurations for separate sub-departments, you should be using a module instead.
   The only difference between your Production and Staging Infrastructure-as-Code should be configuration values set through
   input variables.

1. **Magic strings hardcoded instead of passed as input variables.**
   Email addresses, business unit identifiers, and other environment-specific values must not be hardcoded.
   You wouldn't hard-code these into an application's source code, so keep them as configuration values instead.
   The same is true for Terraform and Helm.
   Do not hard-code membership lists into IAM roles, for example, but pass these values through input variables.
